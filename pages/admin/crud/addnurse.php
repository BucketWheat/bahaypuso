...
if (isset($_POST['addNurse'])) {
    ...
    if (!empty($fname) && !empty($lname) && !empty($address) && !empty($dob) && !empty($age) && !empty($sex) && !empty($licenseNum) && !empty($contactNum) && !empty($email) && !empty($username) && !empty($password)) {

        //convert password to password_hash
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $level = 2;
        //save to database
        $query = "INSERT INTO tbl_nurse (fname, lname, address, dob, age, sex, license_number, contact_number, email, username, password, level, status) VALUES ('$fname', '$lname', '$address', '$dob', $age, '$sex', '$licenseNum', '$contactNum', '$email', '$username', '$hashed_password', $level, '$status')";
