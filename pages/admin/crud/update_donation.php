...
if (isset($_POST['update-cash'])) {
    ...
    if ($status == "Received") {
        ...
        //update status
        $query = "UPDATE tbl_donation SET date_received = '$date', status = '$status' WHERE donation_id = $donation_id";
        $run = mysqli_query($con, $query);

        if ($run) {
            $_SESSION['formSubmitted'] = "updated";
            header('Location: ../donation_pending.php');
            die;
        } else {
...
if (isset($_POST['update-misc'])) {
    ...
    if ($status_misc == "Received") {

        //update status
        $query = "UPDATE tbl_donation SET date_received = '$date', status = '$status_misc' WHERE donation_id = $donation_id_misc";
        $run = mysqli_query($con, $query);
