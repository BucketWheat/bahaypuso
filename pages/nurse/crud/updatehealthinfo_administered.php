...
if (isset($_POST['editHealthinfo_administered'])) {

    //getting update values
    $updateID = $_POST['updateID_administered'];
    ...
    if (!empty($updateID) && !empty($status)) {
        ...
        $query = "UPDATE tbl_healthinfo SET date_administered=NULL, status='$status' WHERE healthinfo_id =$updateID";

        $query_run = mysqli_query($con, $query);

        if ($query_run) {
            if ($level == 2) {
                $update_treatment_query = "UPDATE tbl_treatment SET status='Cancelled' WHERE healthinfo_id = $updateID";
                mysqli_query($con, $update_treatment_query);
